...
func (ctx SofaProtocolBufferCtx) Name() string {
	return "SofaProtocolBuffers"
}

func (ctx SofaProtocolBufferCtx) Init() types.BufferPoolMode {
	return nil
}

func (ctx SofaProtocolBufferCtx) New(interface{}) interface{} {
	buffer := new(SofaProtocolBuffers)
	buffer.HeaderMap = make(map[string]string)
	return buffer
}
...
func (ctx SofaProtocolBufferCtx) Reset(i interface{}) {
	...
	for k := range buf.HeaderMap {
		delete(buf.HeaderMap, k)
	}
}
